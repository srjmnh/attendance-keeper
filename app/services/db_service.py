...
class DatabaseService:
    ...
    # Chat operations
    def log_chat_interaction(self, chat_data):
        """Log a chat interaction"""
        try:
            doc_ref = self.db.collection('chat_logs').document()
            chat_data['id'] = doc_ref.id
            doc_ref.set(chat_data)
            return True
        except Exception as e:
            current_app.logger.error(f"Error logging chat interaction: {str(e)}")
            return False
    ...
    def get_chat_history(self, user_id, limit=50):
        ...
        try:
            history = []
            query = self.db.collection('chat_logs')\
    ...
    def get_chat_analytics(self):
        ...
        try:
            ...
            # Get all chat logs
            logs = self.db.collection('chat_logs').stream()
            user_messages = {}
            ...
            for doc in logs:
                ...
                if user_id:
                    ...
                    if user:
                        analytics['role_distribution'][user.role] += 1
            
            analytics['users_engaged'] = len(user_messages)
            if analytics['users_engaged'] > 0:
                total_messages = sum(user_messages.values())
                analytics['average_messages_per_user'] = total_messages / analytics['users_engaged']
